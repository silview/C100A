...
__s32 get_app_config_key_value(char *child_key)
{
	...
	do{
	    ...
	    eLIBs_strcat(keyname, child_key);  
		
	    ret = esCFG_GetKeyValue_Ex(parser, keyname, (int*)&val, 1);
	    __wrn("after esCFG_GetKeyValue_Ex, ret=%d\n", ret);
	    if(EPDK_OK == ret)
	    {
			eLIBs_printf("-- %s, %d, Get app_config val success!! -- \n", __FILE__, __LINE__);
	    } 
		else
		{
			eLIBs_printf("-- %s, %d, Get app_config val fail!! -- \n", __FILE__, __LINE__);
		}
	    if(parser)
	    {
	...
	eLIBs_printf("-- %s, %d, app_config val = %d -- \n", __FILE__, __LINE__, val);
...
{
	__u32 sys_volume, sys_lang, sys_bright, sys_constract, sys_brightness, sys_saturation, sys_hue, vcom;
...
#if BEETLES_MODIFY_TOOL	
	sys_volume = get_app_config_key_value("volume");
    if(sys_volume < AUDIO_DEVICE_VOLUME_MIN || sys_volume > AUDIO_DEVICE_VOLUME_MAX)
    {
        sys_volume = 26;
    }	
	para->system_para.volume=sys_volume;

	sys_lang = get_app_config_key_value("lang");
    if(sys_lang < EPDK_LANGUAGE_ENM_ENGLISH || sys_lang > EPDK_LANGUAGE_ENM_CHINESET)
    {
        sys_lang = EPDK_LANGUAGE_ENM_CHINESES;
    }
	para->system_para.language=sys_lang;//简体中文

	sys_bright = get_app_config_key_value("light");
    if(sys_bright < LION_BRIGHT_LEVEL1 || sys_bright > LION_BRIGHT_LEVEL15)
    {
        sys_bright = LION_BRIGHT_LEVEL11;
    }	
	para->system_para.backlight=sys_bright;
	
	eLIBs_printf("-- %s, %d, sys_volume = %d, sys_lang = 0x%x, sys_bright = %d -- \n", __FILE__, __LINE__, sys_volume, sys_lang, sys_bright);
#endif	

	para->system_para.style=0;
	para->system_para.backlightoff=0;//不自动关屏
	para->system_para.poweroff=0;//不自动关机
	para->system_para.keytone=2;//按键音是否打开
	para->system_para.echo_delay=0;//echo delay 3
	...
	para->system_para.date.year= 2013;
...
//	para->system_para.tp_adjust_finish_flag = 0;
	...
	sys_constract = get_app_config_key_value("lcd_constract");
	para->system_para.constract	 = sys_constract;

	sys_brightness = get_app_config_key_value("lcd_bright");
	para->system_para.brightness = sys_brightness;

	sys_saturation = get_app_config_key_value("lcd_saturation");
	para->system_para.saturation = sys_saturation;

	sys_hue = get_app_config_key_value("lcd_hue");
	para->system_para.hue = sys_hue;

	vcom = get_app_config_key_value("lcd_vcom");
	para->system_para.vcom = vcom;
	//写入txt文件
	dsk_display_set_lcd_vcom(vcom);
		
	eLIBs_printf("-- %s, %d, sys_constract = %d, sys_brightness = %d, sys_saturation = %d, sys_hue = %d -- \n", __FILE__, __LINE__, sys_constract, sys_brightness, sys_saturation, sys_hue);
#endif	
	...
	para->movie_para.zoom_mode=1;//ROBIN_ZOOM_FULL_SCREEN_SCREEN_RATIO
...
void* dsk_reg_get_para_by_app(reg_app_e eApp)
{
	...
	switch(eApp)
	{
		...
		//case REG_APP_SPORT:			
			//return &all_app_para->para_current.sport_para;
	}
...
#if BEETLES_RESET_LCD_PARAM
	sys_constract = get_app_config_key_value("lcd_constract");
	all_app_para->para_current.system_para.constract	 = sys_constract;
	dsk_display_set_panel_bright(sys_constract);

	sys_brightness = get_app_config_key_value("lcd_bright");
	all_app_para->para_current.system_para.brightness = sys_brightness;
	dsk_display_set_lcd_constract(sys_brightness);

	sys_saturation = get_app_config_key_value("lcd_saturation");
	all_app_para->para_current.system_para.saturation = sys_saturation;
	dsk_display_set_lcd_saturation(sys_saturation);	

	sys_hue = get_app_config_key_value("lcd_hue");
	all_app_para->para_current.system_para.hue = sys_hue;
	dsk_display_set_lcd_hue(sys_hue);

	vcom = get_app_config_key_value("lcd_vcom");
	all_app_para->para_current.system_para.vcom = vcom;
	//写入txt文件
	dsk_display_set_lcd_vcom(vcom);
