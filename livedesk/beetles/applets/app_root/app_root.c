...
{	
	...
	//HTHEME testBmp;
	//__s32 testID[] = {ID_HOME_GREEN_BMP,ID_HOME_WHITE_BMP,ID_HOME_BLACK_BMP,ID_HOME_BLUE_BMP,ID_HOME_RED_BMP};
	//int test = 0;
    if(0)
    {
        __s32 i;
  ...
  //  test_read_nor();
	...
	/*
		 while(1) {
			...
			testBmp = dsk_theme_open(testID[test]);
			test++;
			GUI_LyrWinSel(happ_root_manwin);
			GUI_BMP_Draw(dsk_theme_hdl2buf(testBmp),0,0);
	
			if (test == 4) {
				test = 0;
			}
			esKRNL_TimeDly(500);
			dsk_theme_close(testBmp);
