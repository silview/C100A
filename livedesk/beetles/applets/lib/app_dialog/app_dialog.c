...
#define ID_TIMER_DIALOG_TIMEOUT   1013
__u32 mute_flag=0;//mute 1,
//按键间的间隔
...
static __s32 app_dialog_res_create(APP_DIALOG_RES_T *res, __u32 style, __s32 bmp_id[], __s32 str_id[])
{
	...
	case ADLG_MUTE:
		{
			//get_lang_res(STRING_DIALOG_MUTE, res->str_btn[0]);
			res->btn_result[0] = ADLG_IDMUTE;
			res->btn_count = 1;
		}
		break;		
	default:
...
static __s32 on_dialog_create(__gui_msg_t *msg)
{
	...
	if(mute_flag==0)//sun++ 防止静音图标时消息往此传
	{
		GUI_WinSetFocusChild(msg->h_deswin);
	}
	if (IsDialogTimerValid(wnd_para->timeout))
	{
		ret = GUI_SetTimer(msg->h_deswin, ID_TIMER_DIALOG_TIMEOUT, wnd_para->timeout, NULL);
	}
	return EPDK_OK;
}
...
H_WIN app_dialog_create(H_WIN h_parent, __s32 win_id, __u32 style,
						__s32 bmp_id[], __s32 str_id[], __u32 timeout)
{//TODO:
	dialog_wnd_t *wnd_para;

	__inf("****************************************************************\n");
	__wrn("********  enter dialog app  **************\n");
	__inf("****************************************************************\n");
	mute_flag=0;
...
__s32 app_dialog_ex_destroy(H_WIN h_dialog )
{
	dialog_wnd_t *wnd_para;
	__msg("..............888888888888............\n");
	...
	{
	__msg("..............888888888888............\n");
	GUI_FrmWinDelete(wnd_para->parent);
		__msg("end of dialog\n");
		__msg("..............888888888888............\n");
	}
