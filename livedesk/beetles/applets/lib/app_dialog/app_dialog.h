...
typedef enum tag_APP_DIALOG_E
{
	...
	ADLG_MUTE					= 6 //消息框包含一个按钮 mute  //cai++
}APP_DIALOG_E;
...
{\
	...
	_dialog = app_dialog_create(_parent, _id, (ADLG_TA_CENTER | ADLG_DS_VISIBLE | ADLG_MAKE_WND_STYLE(ADLG_ICONINFORMATION, _btn_s)), _bmp, _str, 0); \
}while (0)

#define default_dialog_timeout(_dialog, _parent, _id, _btn_s, _str, timeout)  do \
{\
	__s32 _bmp[] = {ID_DIALOG_MSGBOX_BMP, ID_DIALOG_MSGBOX_BTN_F_BMP, ID_DIALOG_MSGBOX_BTN_N_BMP, ID_DIALOG_ICON_INFORMATION_BMP}; \
	_dialog = app_dialog_create(_parent, _id, (ADLG_TA_CENTER | ADLG_DS_VISIBLE | ADLG_MAKE_WND_STYLE(ADLG_ICONINFORMATION, _btn_s)), _bmp, _str, timeout); \
}while (0)
