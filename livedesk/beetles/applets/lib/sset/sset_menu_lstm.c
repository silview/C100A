...
#define		COLOR_FOCUS		(APP_COLOR_YELLOW+0x0f)//0xB000A3FF			/* 焦点颜色					 		*/
#define		COLOR_INVALID	(APP_COLOR_GREEN+0x0f)//0xB0A7A7A7			/* 无效颜色					 		*/
#define		COLOR_UFOCUS	(APP_COLOR_GREEN+0x0f)//0xB0575757			/* 非焦点颜色				 		*/
...
// 				   puipara->menu_spin_inc_bmp.w*puipara->menu_spin_inc_bmp.h*4 + 0x100);
		...
		pspin->ftxt_color  = /*GUI_RED*/APP_COLOR_WHITE;
		pspin->uftxt_color = /*GUI_YELLOW*/APP_COLOR_YELLOW;
...
// 				   puipara->menu_lstm_bmp.w*puipara->menu_lstm_bmp.h*4 + 0x100);			
		...
		plitem_para->text_pos.x 	= puipara->menu_lstm_txt.x;
		plitem_para->text_pos.y 	= puipara->menu_lstm_txt.y;
		plitem_para->ftxt_color		= /*GUI_RED*/APP_COLOR_WHITE;
		plitem_para->uftxt_color	= /*GUI_WHITE*/APP_COLOR_WHITE;
...
// 					   puipara->menu_opt_bmp.w*puipara->menu_opt_bmp.h*4 + 0x100);			
	...
	__msg("\n~~~ ctlcreate_info.spClassName = %s ~~~\n", ctlcreate_info.spClassName);
    ...
    GUI_WinSetAddData( msg->h_deswin, (__u32)plist_data );
...
static void  _menu_lstm_command( __gui_msg_t *msg )
{
                ...
                case GUI_MSG_KEY_ENTER:
	                {		
						...
						if(user_para->item_sta == 1)
						{
					        ...
					        notifymsg.dwReserved	= (__u32)user_para;
	...
	else if( (LOSWORD(msg->dwAddData1) > ID_LISTITEM)&&(LOSWORD(msg->dwAddData1) < (ID_LISTITEM + plist_data->item_cnt+1)) )
	{
		...
		if(( ret == 0 )&&(user_para->item_sta == 1))
		{
			...
			notify_msg.dwReserved	= (__u32)user_para;
