...
//onoff: 0 not mute,  1 mute
static __s32 audio_mute_onoff(__s32 onoff)
{
    ES_FILE* fp;

    fp = eLIBs_fopen("b:\\AUDIO\\CTRL", "r+");
    if(fp)
    {
        eLIBs_fioctrl(fp, AUDIO_DEV_CMD_MUTE, onoff, 0);
        eLIBs_fclose(fp);
        fp = NULL;
    }
...
static __s32 audio_mute_get_onoff(void)
{
    __s32 ret;
    ES_FILE* fp;
    ...
    if(fp)
    {
        ret = eLIBs_fioctrl(fp, AUDIO_DEV_CMD_GET_MUTE, 0, 0);
        eLIBs_fclose(fp);
        fp = NULL;
    }
...
{
    #if 1
    robin_play_file(file, tag_info_p);
    #else
	robin_msg_t  *msg_p;
