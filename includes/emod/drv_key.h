...
typedef enum __DRV_KEY_CMD
{
	...
	DRV_KEY_CMD_SET_ECHO_FLAG,
    DRV_KEY_CMD_GET_ECHO_LEVEL,
	...
	DRV_IRKEY_CMD_SET_POWER_FLAGS

}__drv_key_cmd_t;

typedef enum __DRV_RK2_CMD
{
    DRV_RK2_CMD_NULL       =0,
    DRV_RK2_CMD_REG          ,          //挂载回调函数，如果前面存在回调函数，会自动替换前一个回调函数
    DRV_RK2_CMD_UNREG        ,          //卸载回调函数，卸载后，如果前面存在回调函数，会自动使用前一个回调函数
    DRV_RK2_CMD_ECHOFLAG        ,          //卸载回调函数，卸载后，如果前面存在回调函数，会自动使用前一个回调函数
    DRV_RK2_CMD_ECHODATA        ,          //卸载回调函数，卸载后，如果前面存在回调函数，会自动使用前一个回调函数
    DRV_RK2_CMD_
