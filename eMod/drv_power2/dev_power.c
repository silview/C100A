...
static __u32 g_pw_on_period = 2000;//2秒长按则开机(减去1秒，看门狗没复位到系统重启还要一点时间)
...
static __s32 DEV_PowerOnProcess(void)
{
    ...
    //__wrn("create power on timmer ok...\n");

    //让定时器开始跑
    if (esKRNL_TmrStateGet(g_pw_on_timer_hdl) == OS_TMR_STATE_STOPPED)
	{	    
		esKRNL_TmrStart(g_pw_on_timer_hdl);
        __wrn("start power on timmer...\n");
	}

    //__wrn("begin disable monitor and decress freq...\n");

    __power_decr_freq();

    //__wrn("disable monitor and decress freq end...\n");
